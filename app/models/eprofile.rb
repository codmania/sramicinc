class Eprofile < ActiveRecord::Base
  ...
  validates :city, length: {maximum: 100}, if: 'city.present?'
  ...
  validates_format_of :fax, :with => /\A([\+]?[1]{1})?(?:[\(][0-9]{3}[\)]|[0-9]{3})[\-]?[0-9]{3}[\-]?[0-9]{4}\Z/, :message => "should be in format like 123-456-7890 ,(123)456-7890, +11234567890", if: 'fax.present?'
  validates  :overview,length: {maximum: 20000}, if: 'overview.present?'
